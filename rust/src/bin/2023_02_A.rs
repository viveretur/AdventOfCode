...
impl Game {
    fn fits_in(&self, mins: &Game) -> bool {
        (0..3).all(|i| self.max[i] <= mins.max[i])
    }
}

const GAME_MINS: Game = Game {
    id: 0,
    max: [12, 13, 14],
};
...
fn solve(data: &str, mins: &Game) -> usize {
    data.lines()
        .map(parse_line)
        .filter(|g| g.fits_in(mins))
        .map(|g| g.id)
        .sum()
}
...
mod tests {
    ...
    fn examples() {
        ...
        assert_eq!(solve(data, &GAME_MINS), 8);
    }
